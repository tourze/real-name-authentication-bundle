...
class AuthenticationValidationService
{
    ...
    public function validateIdCardFormat(string $idCard): bool
    {
        ...
        if (1 !== preg_match($pattern, $idCard)) {
            return false;
        }
    ...
    public function validateMobileFormat(string $mobile): bool
    {
        ...
        return 1 === preg_match($pattern, $mobile);
    }
    ...
    public function validateCreditCodeFormat(string $creditCode): bool
    {
        ...
        if (1 !== preg_match($pattern, $creditCode)) {
            return false;
        }
    ...
    public function sanitizeInput(array $data): array
    {
        ...
        foreach ($data as $key => $value) {
            if (is_string($value)) {
                ...
                // 对于身份证号，转换最后一位x为X
                if (in_array($key, ['id_card', 'legal_id_card'], true) && 'x' === strtolower(substr($value, -1))) {
                    $value = substr($value, 0, -1) . 'X';
                }

                // 移除银行卡号中的空格和分隔符
                if (in_array($key, ['bank_card', 'bank_account'], true)) {
